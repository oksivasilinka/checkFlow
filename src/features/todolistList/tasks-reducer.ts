...
export const deleteTasks = createAppAsyncThunk<{ id: string; todolistId: string }, DeleteTaskArgs>(
    'tasks/deleteTasks',
    async (arg, thunkAPI) => {
        const { dispatch, rejectWithValue } = thunkAPI
...
export const addTasks = createAppAsyncThunk<{ task: TaskType }, AddTaskArgs>('tasks/addTask', async (arg, thunkAPI) => {
    const { dispatch, rejectWithValue } = thunkAPI
    try {
        dispatch(appActions.setStatus({ status: 'loading' }))
